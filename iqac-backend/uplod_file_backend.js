...
const insertBatchWithRetry = async (batch, maxRetries = 3) => {
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            console.error(`Insert attempt ${attempt + 1} failed:`, err.message);
...
const handleFileUpload = async (file) => {
    ...
    try {
        ...
        // Use OS temp directory if file.tempFilePath doesn't exist
        ...
        const batchSize = 1000; // Reduced batch size for Render
        let insertedCount = 0;
        let skippedCount = 0;
        const errors = [];
        const startTime = Date.now();
        ...
        // Stream parse using ExcelJS
        if (tempFilePath && fs.existsSync(tempFilePath)) {
            const ExcelJS = require('exceljs');
            console.log('Streaming Excel from temp file:', tempFilePath);
                ...
                for await (const row of worksheetReader) {
                    ...
                    try {
                        ...
                        if (normalizedData.length >= batchSize) {
                            ...
                            const result = await insertBatchWithRetry(normalizedData);
                            insertedCount += result.count;
                            normalizedData = [];
                            
                            if (rowsProcessed % (batchSize * 5) === 0) {
                                const rate = (rowsProcessed / ((Date.now() - startTime) / 1000)).toFixed(0);
                                console.log(`Progress: ${rowsProcessed} rows at ${rate} rows/sec`);
                            }
                        }
                        ...
                        console.error(`Error processing row ${rowsProcessed}:`, rowError.message);
                    }
                }
            }
